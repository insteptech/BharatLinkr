...
function CollegeAdminPage(props) {
  ...
  const approvalList = useSelector((data) => data?.adminData?.pendingApprovalList)
  const Heading = ["No.", "Image", "Logo", "Name", "City", "State", "Courses", "Action"]
  const approvalHeading = ["No.", "College", "State", "City", "E-mail", "Action"]
  ...
  const handleApprove = (id) => {
    dispatch(collegeApprovalByAdmin({
      PendingRequest: [
        {
          id: id,
          active: true
          ...
          <>
            <Row>
              <Table responsive className="admin_table" bordered hover>
                ...
                <tbody>
                  {loaderApproval ? <LoaderPage /> : approvalList?.rows?.length > 0 ?
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td className="text-center admin_table_data">
                            ...
                            ></img> */}
                            <Button className="border_btn" onClick={() => console.log('reject')}>Reject</Button>
                            <Button className="border_btn green" onClick={() => handleApprove(item?.id)}>Approve</Button>
                          </td>
                        </tr>
                      );
                    }) : "No college"}
            ...
            <Pagination pagination={pagination} setPagination={setPagination} list={approvalList} />
          </>
        }
      </div>
    </>
