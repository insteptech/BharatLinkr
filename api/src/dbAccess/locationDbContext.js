...
const { Countries, State, City, college } = require('../../models');
...
const countryDelete = async (req) => {
  try {
    ...
    const inState = await State.findOne({
      where: {
        [Op.or]: [
          { countryId: req.id }
    ...
    const inCity = await City.findOne({
      where: {
        [Op.or]: [
          { stateId: req.id },
  ...
  try {
    ...
    await Promise.all(
      body.countryInputs.map(async (item) => {
        ...
        console.log(prg,'9898989')
        if (prg && prg.id !== item.id) {
          throw new Error('Country Already exists');
        } else {
        let res =  await Countries.update(
          { ...item },
          {
            returning: true,
            where: { id: item.id },
      ...
      console.log(result.toString(),'9787878')
    return { data: result,success: true };
  } catch (error) {
    return { data: null, message: error.message, success: false };
  }
